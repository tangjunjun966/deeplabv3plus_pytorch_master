...
def build_scheduler(scheduler_name, optimizer, **kwargs):
    scheduler = None
    if scheduler_name == 'PolyLR':
        max_iter = kwargs.get('max_iter')

        scheduler = PolyLR(optimizer, max_iters=max_iter, power=0.9)
    elif scheduler_name=='step':
        step_size = kwargs.get('step_size')
        if step_size is None:
            step_size=10000
            print('step_size value:{},\t please update new value!!!'.format(step_size))
